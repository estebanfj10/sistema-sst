import streamlit as stimport osimport requestsimport base64from datetime import datetime, timedelta
...
base_dir = "documentos"base_registros = os.path.join(base_dir, "registros")

os.makedirs(base_dir, exist_ok=True)os.makedirs(base_registros, exist_ok=True)
...
tipos = ["ats","caliente","electricidad","espacio confinado","excavacion","general","herramientas","izaje"]
...
for root, _, files in os.walk(base_registros):for f in files:if f.endswith(".pdf"):estado = evaluar_vencimiento(os.path.join(root, f), f)if estado and ("🔴" in estado or "🟡" in estado):alertas.append(f"{estado} - {f}")

if alertas:for a in alertas:st.write(a)else:st.success("✔ Sin alertas")
...
st.markdown("---")st.markdown("## 📊 Panel")

total = len(tipos)docs = sum(len(files) for _, _, files in os.walk(base_registros))

c1, c2 = st.columns(2)c1.metric("Tipos", total)c2.metric("Registros", docs)
...
for t in tipos:st.sidebar.markdown(f"📁 {t}")
